...
def get_preview_layout(size: tuple[int], key: str) -> list[list[sg.Element]]:
    ...
    preview_layout = [
        ...
        [sg.Frame("Header Preview", [
            [sg.Text("Target E-mail address:"), sg.Input(key= "-EMAIL_ADDRESS_PREVIEW-", disabled= True, expand_x= True)],
            [sg.Text("Subject:"), sg.Input(key= "-SUBJECT_PREVIEW-", disabled= True, expand_x= True)],
            [sg.Text("Attachment(s):"), sg.Input(key= "-ATTACHMENT_PREVIEW-", disabled= True, expand_x= True)]
        ], expand_x= True)],
...
def get_target_email_address(values, excel_sheet, preview_index):
    '''Returns the target e-mail address preview field according to user input'''
    
    target_email_address = ""
    column_name = values["-TARGET_EMAIL_ADDRESS-"]
    df_column = excel_sheet[column_name]
    target_email_address = df_column[preview_index]
    return target_email_address

def get_subject(values: dict, placeholders: list[str], excel_sheet: pd.DataFrame, preview_index: int, window: sg.Window) -> None:
...
def get_attachment_filenames(values, excel_sheet, preview_index):
...
def show_preview_event(placeholders: list[str], excel_sheet: pd.DataFrame, values: dict, template_text: str, window: sg.Window) -> tuple[int, bool, tk.Widget]:
    ...
    if placeholders:
        ...
        window.Element("-EMAIL_ADDRESS_PREVIEW-").update(get_target_email_address(values, excel_sheet, preview_index))
        window.Element("-SUBJECT_PREVIEW-").update(get_subject(values, placeholders, excel_sheet, preview_index, window))
        window.Element("-ATTACHMENT_PREVIEW-").update(get_attachment_filenames(values, excel_sheet, preview_index))
